		...
		var ChatLauncher = A.Component.create(
			{
				...
				prototype: {
					...
					_getChatLink: function(chatLinkConfig) {
						var instance = this,
							type = chatLinkConfig.type,
							URIFn = instance._chatURIFnMap[chatLinkConfig.client];

						var href = URIFn.apply(instance, [chatLinkConfig.topic, type, chatLinkConfig.users]);
					...
					_getChatLinkGroupLabel: function(chatLinkConfig) {
						var instance = this,
							users = chatLinkConfig.users,
							label = users;
							multipleParticipants = instance._hasMultipleParticipants(users);

						var sms = (chatLinkConfig.client === 'sms');

						if (multipleParticipants && !sms) {
							if (instance._isPhoneNumber(users)) {
								label = 'Phone Numbers';
							}
							else {
								var topic = chatLinkConfig.topic;

								label = instance._adjustLabelCase((topic !== 'false') ? topic : instance._activeTrigger.html());
							}
						}
						else if (sms) {
							label = 'SMS (Native)';
					...
					_getChatLinkTypes: function(client, users) {
						var instance = this,
							types;

						if (client === 'skype') {
							types = instance._isPhoneNumber(users) ? SKYPE_PHONE_TYPES : SKYPE_USERNAME_TYPES;
						}
						else if (client === 'sms') {
							types = [client];
						}
					...
					_getSkypeURI: function(topic, type, users) {
						var instance = this,
							conferenceTopic,
							uri = ['skype:'];
					...
					_getSMSURI: function(topic, type, users) {
						return 'sms:' + users;
					},
